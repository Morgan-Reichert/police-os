...
export default function Interception() {
  ...
  useEffect(() => {
    api.listWiretaps().then(setWiretaps).catch(() => setWiretaps([]));
  }, []);

  useEffect(() => {
    if (!selected || !waveRef.current) return;

    // Nettoyage ancien wavesurfer
    ...
    const ws = WaveSurfer.create({
      ...
      barWidth: 2,
      barGap: 1,
      url: api.wiretapUrl(selected.id),
    });
        ...
        {wiretaps.map((w) => (
          ...
          >
            <div className="flex items-center gap-2 text-sm text-white">
              <Radio size={12} className="text-accent-red blink" />
              {w.suspect}
            </div>
          ...
          <>
            ...
            <div className="border border-bureau-500 bg-black/40 p-3 mb-3">
              ...
              <div className="grid grid-cols-[110px_1fr] gap-y-1 text-sm">
                <div className="text-bureau-300">Cible :</div>
                <div className="text-white font-bold">{selected.suspect}</div>
                <div className="text-bureau-300">Ligne :</div>
                <div className="text-white font-mono">{selected.ligne}</div>
                <div className="text-bureau-300">Captation :</div>
                <div className="text-white">{selected.date}</div>
                <div className="text-bureau-300">Durée :</div>
                <div className="text-white">{selected.duree}</div>
            ...
            <div className="border border-bureau-500 bg-black p-3 flex-1">
              ...
              <div className="flex items-center gap-3">
                <button
                  onClick={togglePlay}
                  className="w-12 h-12 rounded-full bg-accent-amber text-black flex items-center justify-center
                             hover:bg-accent-amber/80 transition"
                >
              ...
              {selected.transcription && (
                <div className="mt-4 p-3 bg-bureau-800 text-xs border-l-2 border-accent-blue max-h-40 overflow-auto">
                  <div className="text-accent-blue uppercase mb-1">Transcription (brouillon)</div>
